...
public class MemberService {
    ...
    public void signupMember(MemberSignupRequestDTO dto) {
        ...
        // 전화번호 인증 여부 검사
        PhoneNumberVerification phoneNumberVerification = phoneNumberVerificationRepository.findByPhoneNumber(dto.getPhoneNumber())
                .orElseThrow(() -> new BadRequestException(ErrorStatus.BAD_REQUEST_MISSING_PHONE_NUMBER_VERIFICATION.getMessage()));
    ...
    public MemberLoginResponseDTO loginMember(MemberLoginRequestDTO dto) {
        ...
        // JWT 토큰 생성 (액세스, 리프레쉬)
        Map<String, String> tokens = jwtService.createAccessAndRefreshToken(member.getId(), member.getEmail(), member.getRole());
